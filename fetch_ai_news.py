...
# Raw text storage root
RAW_ROOT = "/opt/data/cron/raw"
...
def download_and_extract_pdf(arxiv_id, max_pages=5):
    """Download arXiv PDF, extract text, save to raw/. Returns (full_text, raw_path)."""
...
def fetch_arxiv_papers_with_fulltext(max_results=8):
    ...
    items = []
    # Focused LLM-related queries (精简: 2个查询 × 3篇 = ~6篇)
    queries = [
        ("ti:language+model+OR+ti:LLM+OR+ti:transformer+OR+ti:GPT+OR+ti:chat", "LLM-Core"),
        ("ti:agent+AND+(cat:cs.CL+OR+cat:cs.AI)+OR+ti:reasoning+AND+cat:cs.CL+OR+ti:RAG", "LLM-Agent"),
    ]
    ...
    for cat_query, label in queries:
        ...
        for arxiv_id in paper_ids[:max_results]:
            ...
            try:
                ...
                full_text, raw_path = download_and_extract_pdf(arxiv_id, max_pages=4)
...
def fetch_openalex_papers(max_results=5):
    ...
    try:
        ...
        for work in result.get('results', []):
            ...
            # Save abstract as raw text for non-arXiv papers
            raw_path = None
            if abstract:
...
def main():
    ...
    # Collect raw file paths for reference
    raw_files = {}
    for p in papers:
        rp = p.get('raw_text_path')
        if rp:
            raw_files[p.get('arxiv_id', p['title'][:40])] = rp
    ...
    output = {
        ...
        "raw_files": raw_files,  # LLM: read these for selected papers
        "all_news": all_news,
        "papers": papers,
    }
